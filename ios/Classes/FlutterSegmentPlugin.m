      ...
      next([context
        modify: ^(id<SEGMutableContext> _Nonnull ctx) {
          ...
          @try {
            ...
            NSDictionary *combinedContext = [SegmentFlutterPlugin
...
- (void)setContext:(FlutterMethodCall*)call result:(FlutterResult)result {
  ...
  @catch (NSException *exception) {
    result([FlutterError
      errorWithCode:@"SegmentFlutterException"
...
- (void)track:(FlutterMethodCall*)call result:(FlutterResult)result {
  ...
  @catch (NSException *exception) {
    result([FlutterError errorWithCode:@"SegmentFlutterException" message:[exception reason] details: nil]);
  }
}
...
+ (NSDictionary *) mergeDictionary: (NSDictionary *) first with: (NSDictionary *) second {
  ...
  [second enumerateKeysAndObjectsUsingBlock:^(id key, id value, BOOL *stop) {
    ...
    if (!contained) {
      ...
      [result setObject:[SegmentFlutterPlugin mergeDictionary:result[key] with:value]
        forKey:key];
    }
  }];
  return result;
